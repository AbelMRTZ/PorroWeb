...
// 🚀 Nueva función: Comprueba si han transcurrido 3 horas o más desde el inicio del partido
...
export default function AdminPorra() {
  ...
  const [archivadosOpen, setArchivadosOpen] = useState(false) // 🚀 Estado para el acordeón
  ...
  return (
    <div className="page">
      <div className="porra-container">
        ...
        {/* 🚀 Sección Acordeón: Partidos Archivados (> 3h) - AHORA ARRIBA */}
        {partidosFaseGrupos.filter(p => comprobarPartidoArchivado(p.timestamp)).length > 0 && (
          <div className="archivados-container">
            <button 
              ...
              onClick={() => setArchivadosOpen(!archivadosOpen)}
            >
              <i className={`fa-solid ${archivadosOpen ? 'fa-chevron-up' : 'fa-chevron-down'}`}></i>
              {archivadosOpen ? 'Ocultar Partidos Finalizados' : `Ver Partidos Finalizados para puntuar (${partidosFaseGrupos.filter(p => comprobarPartidoArchivado(p.timestamp)).length})`}
            </button>
